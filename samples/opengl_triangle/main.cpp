  ...
  try {
    ...
    const auto windowId =
        windowSystem.createWindow(engine::platform::WindowCreateInfo{
            .title = "QumaRenderer - Mesh Engine (.obj + Lighting)",
            .size = {1280, 720},
            .resizable = true,
            .highDpi = true});
    ...
    while (running && !windowSystem.shouldClose(windowId)) {
      ...
      rotation +=
          rotationSpeed * static_cast<float>(deltaSeconds) * 0.0174532925f;
      ...
      ImGui::SliderFloat3("Light Position", lighting.lightPosition, -8.0f,
                          8.0f);
